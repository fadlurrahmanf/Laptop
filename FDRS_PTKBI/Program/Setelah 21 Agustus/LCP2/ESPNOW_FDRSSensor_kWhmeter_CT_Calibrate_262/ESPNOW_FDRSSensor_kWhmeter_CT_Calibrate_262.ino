...
uint16_t timeout = 1000;   //Serial timeout[ms]
const float Vref = 1.218;  //[V]
// const float RL = 0.5;           //[mOhm]
// const float R1 = 0.5230;        //[kOhm]
// const float R2 = 5 * 390.0;     //[kOhm]
float R8 = 5 * 20;          //kOhm, Rsum
float R7 = 0.0249;          //kOhm
// float R5 = 3.3;          // Ohm
float R5 = 0.3;             // Ohm
float Rt = 1000;            // CT spec
uint16_t Hz = 50;           //[Hz]
uint16_t updateRate = 400;  //[ms]

uint16_t oneKwh = 40000;

bool rlyStatus = true;
bool changeRly = true;

uint8_t updatePeriode = 60;
...
////////////////////////////////////////////////////////////////////////////////
int requestData(int /*argc*/ = 0, char ** /*argv*/ = NULL) {
  sendSta = true;
  return 0;
};
...
////////////////////////////////////////////////////////////////////////////////
int setOnekwh(int argc, char **argv) {
  if (argc != 2) {
    shell.println("bad argument count");
    return -1;
  }
  id.oneKwh = atoi(argv[1]);
  shell.print("Setting oneKwh to ");
  shell.println(id.oneKwh);
  updateEEPROM = true;
  return EXIT_SUCCESS;
}

////////////////////////////////////////////////////////////////////////////////
int setRt_CT(int argc, char **argv) {
  if (argc != 2) {
    shell.println("bad argument count");
    return -1;
  }
  id.Rt_CT = atof(argv[1]);
  shell.print("Setting Rt_CT to ");
  shell.println(id.Rt_CT);
  updateEEPROM = true;
  return EXIT_SUCCESS;
}

////////////////////////////////////////////////////////////////////////////////
int setR5_CT(int argc, char **argv) {
  if (argc != 2) {
    shell.println("bad argument count");
    return -1;
  }
  id.R5_CT = atof(argv[1]);
  shell.print("Setting R5_CT to ");
  shell.println(id.R5_CT);
  updateEEPROM = true;
  return EXIT_SUCCESS;
}

////////////////////////////////////////////////////////////////////////////////
int setcoef_current(int argc, char **argv) {
  if (argc != 2) {
    shell.println("bad argument count");
    return -1;
  }
  id.coef_current = atof(argv[1]);
  VIcoef = VIcoef * id.coef_current;
  shell.print("Setting coef_current to ");
  shell.println(id.coef_current,15);
  updateEEPROM = true;
  return EXIT_SUCCESS;
}

////////////////////////////////////////////////////////////////////////////////
int setcoef_voltage(int argc, char **argv) {
  if (argc != 2) {
    shell.println("bad argument count");
    return -1;
  }
  id.coef_voltage = atof(argv[1]);
  VIcoef = VIcoef * id.coef_voltage;

  shell.print("Setting coef_voltage to ");
  shell.println(id.coef_voltage);
  updateEEPROM = true;
  return EXIT_SUCCESS;
}

////////////////////////////////////////////////////////////////////////////////
int setcoef_watt(int argc, char **argv) {
  if (argc != 2) {
    shell.println("bad argument count");
    return -1;
  }
  id.coef_watt = atof(argv[1]);
  shell.print("Setting coef_watt to ");
  shell.println(id.coef_watt);
  updateEEPROM = true;
  return EXIT_SUCCESS;
}

////////////////////////////////////////////////////////////////////////////////
int setcoef_kwh(int argc, char **argv) {
  if (argc != 2) {
    shell.println("bad argument count");
    return -1;
  }
  id.coef_kwh = atof(argv[1]);
  shell.print("Setting coef_kwh to ");
  shell.println(id.coef_kwh);
  updateEEPROM = true;
  return EXIT_SUCCESS;
}
...
int getInfo(int /*argc*/, char ** /*argv*/) {
  shell.print("typeId is ");
  shell.println(id.typeId);
  ...
  shell.print("mode is ");
  shell.println(id.mode);
  shell.print("periode is ");
  shell.println(id.periode);
  shell.print("counter is ");
  shell.println(id.counter);
  shell.print("Rt_CT is ");
  shell.println(id.Rt_CT);
  shell.print("R5_CT is ");
  shell.println(id.R5_CT);
  shell.print("coef_current is ");
  shell.println(id.coef_current,15);
  shell.print("coef_voltage is ");
  shell.println(id.coef_voltage,15);
  shell.print("coef_watt is ");
  shell.println(id.coef_watt,15);
  shell.print("coef_kwh is ");
  shell.println(id.coef_kwh,15);
  return EXIT_SUCCESS;
}
...
void setup() {
  ...
  lastCounter = id.counter;
  // start program randomly
  int delayRnd = random(200);
  delay(delayRnd * 100);
  ...
  shell.addCommand(F("requestData"), requestData);
  ...
  shell.addCommand(F("setOnekwh <counter>"), setOnekwh);
  shell.addCommand(F("setRt_CT <Rt_CT>"), setRt_CT);
  shell.addCommand(F("setR5_CT <R5_CT>"), setR5_CT);
  shell.addCommand(F("setcoef_current <coef_current>"), setcoef_current);
  shell.addCommand(F("setcoef_voltage <coef_voltage>"), setcoef_voltage);
  shell.addCommand(F("setcoef_watt <coef_watt>"), setcoef_watt);
  shell.addCommand(F("setcoef_kwh <coef_kwh>"), setcoef_kwh);
...
void loop() {
  ...
  if (id.mode == 1) {
    if ((millis() - lastUpdate) > updateRateSend) {
      lastUpdate = millis() - random(10000);
      sendStatus();
    }
  }
...
bool getCurrent(float *current) {
  ...
  *current = ((float)data * Vref / ((324004.0 * (R5 * 1000) / Rt))) * id.coef_current;
  return true;
}
...
bool getVoltage(float *voltage) {
  ...
  *voltage = ((float)data * Vref * (R8) / (79931.0 * R7 * 1000)) * id.coef_voltage;
  return true;
}
...
bool getActivePower(float *activePower) {
  ...
  if (rowActivePower < 0)
    rowActivePower = -rowActivePower;
  *activePower = ((float)rowActivePower * Vref * Vref * (R8) / (4046.0 * ((R5 * 1000) / Rt) * 1000.0 * R7)) * id.coef_watt * VIcoef;
  return true;
}
...
bool getActiveEnergy(float *activeEnergy) {
  ...
  //Serial.print("Float de Energia: ");
  //Serial.println(rowCF_CNT);
  *activeEnergy = ((float)rowCF_CNT * 1638.4 * 256.0 * Vref * Vref * (R8) / (3600000.0 * 4046.0 * ((R5 * 1000.0) / Rt) * R7 * 1000)) * id.coef_kwh;
