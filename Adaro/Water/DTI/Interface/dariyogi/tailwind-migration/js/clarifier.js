...
function UpdateDataDisplay() {
    ...
    http.open("GET", "http://192.168.0.101:1880/StatusRelay?data=" + String(intervalcounter), true);
...
function SendCommand(address) {
    ...
    http.open("GET", "http://192.168.0.101:1880/SendCommand?address=" + String(address) + "&cmd=" + cmd, true);
    http.send();
    http.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            var data = JSON.parse(this.responseText);
...
function toggleRadio(thisID, otherID) {
    ...
    if (clickedCheckbox.checked) {
        ...
        var url = "http://192.168.0.101:1880/getMode?";
...
function buttonManual(buttonId) {
    ...
    if (button.textContent === "OFF") {
        ...
        // alert("on");
        http.open("GET", "http://192.168.0.101:1880/getTombol?" + buttonId + "=1", true);
        http.send();
    } else {
        ...
        // alert("off");
        http.open("GET", "http://192.168.0.101:1880/getTombol?" + buttonId + "=0&Durasi=" + currentTime[buttonId], true);
        http.send();
    }
}
...
function btnCheck(buttonId) {
    ...
    var http = new XMLHttpRequest();
    http.open("GET", "http://192.168.0.101:1880/getCheck?id=" + String(buttonId), true);
    http.send();
    http.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
            var data = JSON.parse(this.responseText);
...
function btnSubmit(buttonId) {
    ...
    var http = new XMLHttpRequest();
    var url = "http://192.168.0.101:1880/getExecute?ID=" + buttonId + "&Data=" + JSON.stringify(data);
    http.open("GET", url, true);
    http.send();
    http.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
...
function btnStop(buttonId) {
    ...
    buttonElement.textContent = "Done!";
    var http = new XMLHttpRequest();
    var url = "http://192.168.0.101:1880/getExecute?stop=0&ID=" + buttonId;
    http.open("GET", url, true);
    http.send();
    http.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
...
function btnRun(buttonId) {
    ...
    var http = new XMLHttpRequest();
    var url = "http://192.168.0.101:1880/getExecute?run=1&ID=" + buttonId;
    http.open("GET", url, true);
    http.send();
    http.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
