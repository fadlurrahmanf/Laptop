...
const byte CalibrationSlopeLow[] = { 0x06, 0x06, 0x10, 0x02, 0x00, 0x00, 0x2D, 0x7D };
// 0x06,0x06,0x10,0x02,0x00,0x00,0x2D,0x7D
const byte CalibrationSlopeHigh[] = { 0x06, 0x06, 0x10, 0x04, 0x00, 0x00, 0xCD, 0x7C };
// 0x06,0x06,0x10,0x04,0x00,0x00,0xCD,0x7C
...
bool makeMeasurement() {
  ...
  if ((buf[0] == 0x06) && (buf[1] == 0x03)) {
    int rawPHS = buf[3] * 256 + buf[4];
    decPHS = buf[5] * 256 + buf[6];
...
bool calZero() {
  ...
  if ((buf[0] == 0x06) && (buf[1] == 0x06)) {  // PHS100,0,111,0
    int rawPHS = buf[3] * 256 + buf[4];
    decPHS = 111;  //buf[5] * 256 + buf[6];
    int rawT = buf[7] * 256 + buf[8];
    int decT = buf[9] * 256 + buf[10];
    PHS = 0;   //rawPHS;
    Temp = 0;  //rawT;
...
bool calSlopeLow() {  // PHS100,1,111,1
  serial_flush();
  for (byte i = 0; i < 8; i++) {
    Serial.write(CalibrationSlopeLow[i]);
    delay(5);
  }
...
bool calSlopeHigh() {  // PHS100,1,111,1
  serial_flush();
  for (byte i = 0; i < 8; i++) {
    Serial.write(CalibrationSlopeHigh[i]);
    delay(5);
  }
...
void setup() {
  ...
  if (config_code == 47) {
    ...
    EEPROM.write(devid_addr, 102);  // default devid address
...
void loop() {
  ...
  if (!modeSta) {
    ...
    if ((now - lastUpdate) > updateRate) {
      ...
      if (sta) {
        msg = String(DEVID) + "," + String(PHS) + "," + String(decPHS) + "," + String(Temp);
        sendDataLora(msg);
      } else {
        ...
        if (dataValid) {
          ...
          if (devid == DEVID) {
            ...
            if (cmd == "4") {  //mode
              ...
              } else if (getValue(str, ',', 2) == "3") {
                String msg = "";
                bool sta2 = calSlopeHigh();
                if (sta2) {
                  msg = String(DEVID) + "," + String(PHS) + "," + String(decPHS) + "," + String(Temp);
                  delay(1000);
                  sendDataLora(msg);
