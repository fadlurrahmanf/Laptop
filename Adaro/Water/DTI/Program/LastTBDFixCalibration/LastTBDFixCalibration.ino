...
const byte CalibrationSlope[] = { 0x06, 0x06, 0x10, 0x00, 0x00, 0x00, 0x8C, 0xBD };
//: 0x06,0x06,0x10,0x00,0x00,0x00,0x8C,0xBD
const byte CalibrationZero[] = { 0x06, 0x06, 0x10, 0x04, 0x27, 0x10, 0xD7, 0x40 };
//: 0x06,0x06,0x10,0x04,0x27,0x10,0xD7,0x40
...
bool makeMeasurement() {
  ...
  if ((buf[0] == 0x06) && (buf[1] == 0x03)) {
    int rawTBD = buf[3] * 256 + buf[4];
    decTBD = buf[5] * 256 + buf[6];
...
void loop() {
  ...
  if (!modeSta) {
    ...
    if ((now - lastUpdate) > updateRate) {
      ...
      if (sta) {
        msg = String(DEVID) + "," + String(tbd) + "," + String(decTBD) + "," + String(Temp);
        sendDataLora(msg);
      } else {
  ...
  if (receivedFlag) {
    ...
    if (transmitDone) {      // habis transmit atau engga..
      transmitDone = false;  //jika transmit maka do nothing
    } else {                 // jika receive dan bukan transmit..
      // you can read received data as an Arduino String
      String str = "";                 //ID,CMD,Vparams
      int state = lora.readData(str);  //     "TBD100,0,1#:CRC"
      Serial.println(str);
      ...
      if (state == ERR_NONE) {
        ...
        if (dataValid) {
          ...
          if (devid == DEVID) {
            ...
            if (cmd == "0") {  // Request Data = harus ngasih feedback data
              String msg = "";
              bool sta = makeMeasurement();
              if (sta) {
                msg = String(DEVID) + "," + String(tbd) + "," + String(decTBD) + "," + String(Temp);  //TBD100,245,2,32
                delay(1000);
                sendDataLora(msg);
              } else {
                msg = String(DEVID) + ",error";
            ...
            if (cmd == "4") {  //mode
              if (getValue(str, ',', 2) == "1") // TBD100,4,Vparam
              {
