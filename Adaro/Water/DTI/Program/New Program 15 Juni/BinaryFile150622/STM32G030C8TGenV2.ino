...
void loop() {
  ...
  if (!modeSta) {
    ...
    if ((now - lastUpdate) > updateRate) {
      ...
      bool sta = true;
      if (sta) {
        int current_mA = ina.getCurrent() * 100000;
        msg = String(DEVID) + "," + String(current_mA); // + "," + String(Pulse_Count);
        sendDataLora(msg);
      } else {
      ...
      if (state == ERR_NONE) {
        ...
        if (dataValid) {
          ...
          if (devid == DEVID) {
            ...
            if (cmd == "0") {
              int current_mA = ina.getCurrent() * 100000;
              String msg = String(DEVID) + "," + String(current_mA);
              Serial.println(msg);
              delay(1000);
              sendDataLora(msg);
            }
